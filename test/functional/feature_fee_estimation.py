    ...
    def sanity_check_rbf_estimates(self, utxos):
        ...
        for _ in range(5):
            ...
            for _ in range(45):
                ...
                # De-prioritise the transactions on the miner so they aren't accepted to the miner's mempool
                miner.prioritisetransaction(tx["txid"], fee_delta=-COIN)
                txs.append(tx)
            # Broadcast 5 low fee transaction which don't need to be accepted by the miner
            non_rbf_txids = []
            for _ in range(5):
