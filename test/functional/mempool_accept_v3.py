...
class MempoolAcceptV3(BitcoinTestFramework):
    ...
    def test_v3_ancestors_package(self):
        ...
        assert_equal(result['package_msg'], f"v3-violation, v3 child tx {tx_v3_child_heavy['wtxid']} is too big: {tx_v3_child_heavy['tx'].get_vsize()} > 1000 virtual bytes")
        self.check_mempool([])
        ...
        assert all([txresult["package-error"] == f"v3-violation, tx {tx_v3_grandchild['wtxid']} would have too many ancestors" for txresult in result])
    ...
    def test_v3_in_testmempoolaccept(self):
        ...
        expected_error_v3_from_v2 = f"v3-violation, v3 tx {tx_v3_from_v2['wtxid']} cannot spend from non-v3 tx {tx_v2['wtxid']}"
        ...
        expected_error_v2_from_v3 = f"v3-violation, non-v3 tx {tx_v2_from_v3['wtxid']} cannot spend from v3 tx {tx_v3['wtxid']}"
        ...
        expected_error_2children = f"v3-violation, tx {tx_v3_parent['wtxid']} would exceed descendant count limit"
        ...
        expected_error_extra = f"v3-violation, tx {tx_v3_parent['wtxid']} would exceed descendant count limit"
