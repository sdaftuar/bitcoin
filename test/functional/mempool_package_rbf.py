...
class PackageRBFTest(BitcoinTestFramework):
    ...
    def test_wrong_conflict_cluster_size_linear(self):
        ...
        package_hex1, package_txns1 = self.create_simple_package(coin1, DEFAULT_FEE, DEFAULT_FEE + 1)
        package_hex2, package_txns2 = self.create_simple_package(coin2, DEFAULT_FEE, DEFAULT_FEE + 1)
        package_hex3, package_txns3 = self.create_simple_package(coin3, DEFAULT_FEE, DEFAULT_FEE + 1)

        for package_hex in [package_hex1, package_hex2, package_hex3]:
            package_result = node.submitpackage(package_hex)
            assert_equal(package_result["package_msg"], "package RBF failed: unable to compute mining score")
