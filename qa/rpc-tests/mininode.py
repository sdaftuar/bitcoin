...
class CBlockHeader(object):
    ...
    def __repr__(self):
        return "CBlockHeader(nVersion=%i hashPrevBlock=%064x hashMerkleRoot=%064x nTime=%s nBits=%08x nNonce=%08x)" % (self.nVersion, self.hashPrevBlock, self.hashMerkleRoot, time.ctime(self.nTime), self.nBits, self.nNonce)
...
# number of entries
# vector of hashes
# hash_stop (hash of the last desired block header, 0 to get as many as possible)
...
class msg_headers(object):
    ...
    def __init__(self):
        self.headers = []
    def deserialize(self, f):
        blocks = deser_vector(f, CBlock) # comment in bitcoind indicates these are blocks
        for x in blocks:
...
class msg_reject(object):
    ...
    def serialize(self):
        ...
        return r
    def __repr__(self):
        return "msg_reject: %s %d %s [%064x]" % (self.message, self.code, self.reason, self.data)


# This is what a callback should look like for NodeConn
class NodeConnCB(object):
    # Derived classes should call this function to set the message map
    # which associates the derived classes' functions to incoming messages
    def __init__(self):
        # Acquire on all callbacks -- overkill for now
        # since asyncore is single-threaded
        self.cbLock = Lock() 
    ...
    def deliver(self, node, message):
        with self.cbLock:
            try:
                self.cbmap[message.command](node, message)
            except:
                print "ERROR delivering %s (%s)" % (repr(message), sys.exc_info()[0])

    def on_version(self, node, message):
        if message.nVersion >= 209:
            node.send_message(msg_verack())
        node.ver_send = min(MY_VERSION, message.nVersion)
        if message.nVersion < 209:
            node.ver_recv = node.ver_send
    def on_verack(self, node, message):
        node.ver_recv = node.ver_send
    ...
    def on_addr(self, node, message): pass
    def on_alert(self, node, message): pass
    def on_getdata(self, node, message): pass
    def on_getblocks(self, node, message): pass
    def on_tx(self, node, message): pass
    def on_block(self, node, message): pass
    def on_getaddr(self, node, message): pass
    def on_headers(self, node, message): pass
    def on_getheaders(self, node, message): pass
    def on_ping(self, node, message): pass
    def on_reject(self, node, message): pass
    def on_close(self, node): pass
...
class NodeConn(asyncore.dispatcher):
    ...
    def handle_close(self):
        print " Closing Connection to %s:%d... bye :)" % (self.dstaddr, self.dstport)
        self.state = "closed"
        self.recvbuf = ""
        self.sendbuf = ""
        try:
    ...
    def got_data(self):
        while True:
            if len(self.recvbuf) < 4:
                return
            if self.recvbuf[:4] != self.MAGIC_BYTES["regtest"]:
            ...
            if command in self.messagemap:
                ...
                self.show_debug_msg("Unknown command: '" + command + "' " + repr(msg))
...
class BlockStore(object):
    ...
    def headers_for(self, locator, hash_stop, current_tip=None):
        ...
        if current_tip not in self.blockMap:
            return None
        response = msg_headers()
        headersList = [ CBlockHeader(self.blockMap[current_tip]) ]
        maxheaders = 2000
        while (headersList[0].sha256 not in locator.vHave):
            prevBlockHash = headersList[0].hashPrevBlock
            if prevBlockHash in self.blockMap:
                headersList.insert(0, CBlockHeader(self.blockMap[prevBlockHash]))
            else:
                break
    ...
    def have_block(self, hash):
        return hash in self.blockMap
    ...
    def get_blocks(self, inv):
        responses = []
        for i in inv:
            if i.hash in self.blockMap:
                responses.append(msg_block(self.blockMap[i.hash]))
        return responses
