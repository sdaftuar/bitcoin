...
class TestNode(NodeConnCB):
    ...
    def on_getdata(self, node, message):
        ...
        [node.send_message(r) for r in self.tx_store.get_transactions(message.inv)]
    ...
    def send_inv(self, obj):
        ...
        self.node.send_message(msg_inv([CInv(mtype, obj.sha256)]))

    def send_getheaders(self):
        # We ask for headers from their last tip.
        m = msg_getheaders()
        m.locator = self.block_store.get_locator(self.bestblockhash)
        self.node.send_message(m)

    # This assumes BIP31
    def send_ping(self, nonce):
        self.pingMap[nonce] = True
        self.node.send_message(msg_ping(nonce))
    ...
    def send_mempool(self):
        self.lastInv = []
        self.node.send_message(msg_mempool())
...
class TestManager(object):
    ...
    def add_all_connections(self, nodes):
        for i in range(len(nodes)):
            ...
            self.connections[-1].cb.add_node(self.connections[-1])
    ...
    def sync_blocks(self, blockhash, num_blocks):
        ...
        while max_tries > 0:
            results = [ blockhash in c.cb.block_request_map and c.cb.block_request_map[blockhash] for c in self.connections ]
    ...
    def sync_transaction(self, txhash, num_events):
        ...
        while max_tries > 0:
            results = [ txhash in c.cb.tx_request_map and c.cb.tx_request_map[txhash] for c in self.connections ]
            if False not in results:
                break
            time.sleep(0.05)
            max_tries -= 1
    ...
    def run(self):
        ...
        test_instance = self.test_generator.get_next_test_case(counter)
        while test_instance is not None:
            block = None
            block_outcome = None
            tx = None
            tx_outcome = None
            ...
            for b_or_t, outcome in test_instance.blocks_and_transactions:
                if isinstance(b_or_t, CBlock):
                    ...
                    # Send inv's to each node for each block
                    if (test_instance.sync_every_block):
                        [ c.cb.send_inv(block) for c in self.connections ]
                        self.sync_blocks(block.sha256, 1)
                        if (not self.check_results(block.sha256, outcome)):
                            raise AssertionError("Test failed at test %d" % counter)
                    else:
                    ...
                    tx = b_or_t
                    tx_outcome = outcome
                    # Add to shared tx store, inv to all nodes
            ...
            if (not test_instance.sync_every_block and block is not None):
                ...
                if (not self.check_results(block.sha256, block_outcome)):
                    raise AssertionError("Block test failed at test %d" % counter)
            if (not test_instance.sync_every_tx and tx is not None):
                self.sync_transaction(tx.sha256, len(test_instance.blocks_and_transactions))
                if (not self.check_mempool(tx.sha256, tx_outcome)):
                    raise AssertionError("Mempool test failed at test %d" % counter)

            print "Test %d: PASS" % counter, [ c.rpc.getblockcount() for c in self.connections ]
            counter += 1
            test_instance = self.test_generator.get_next_test_case(counter)
