...
class SegWitTest(BitcoinTestFramework):
    ...
    def test_witness_tx_relay_before_segwit_activation(self):
        ...
        try:
            ...
            print "\tERROR: Duplicate tx getdata not yet fixed! Continuing anyway for now"
            #XXX assert(False)
    ...
    def test_tx_relay_after_segwit_activation(self):
        print "\tTesting relay of witness transactions after segwit activation"
        # Generate a transaction that doesn't require a witness, but send it
    ...
    def test_signature_version_1(self):
        print "\tTesting signature version 1 (used by segwit)"
        key = CECKey()
        ...
        # Split the utxo into 100 outputs.
        ...
        tx = CTransaction()
        tx.vin.append(CTxIn(COutPoint(prev_utxo.sha256, prev_utxo.n), ""))
        split_value = int(prev_utxo.nValue / NUM_TESTS)
        for i in xrange(NUM_TESTS):
    ...
    def test_p2sh_witness(self, segwit_activated):
        ...
        # post-segwit, because before segwit, it fails CLEANSTACK, while after
        # segwit, it needs a witness.
