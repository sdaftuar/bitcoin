...
class BIP91Test(BitcoinTestFramework):
    ...
    def setup_network(self):
        self.nodes = []
        self.nodes.append(start_node(0, self.options.tmpdir, ["-debug", "-whitelist=127.0.0.1", "-bip91height=288"]))
        self.nodes.append(start_node(1, self.options.tmpdir, ["-debug", "-whitelist=127.0.0.1", "-bip91height=288"])) # connect to a dummy node to allow getblocktemplate
    ...
    def run_test(self):
        ...
        tmpl = self.nodes[0].getblocktemplate({})
        assert_equal(tmpl['version'], 0x10000001|0x20000002) # 0x10000001 are TESTDUMMY and CSV
