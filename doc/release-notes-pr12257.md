...
- A new `-avoidpartialspends` flag has been added (default=false). If enabled, the wallet will try to spend UTXO's that point at the same destination
  ...
  coin selections). Wallets with this flag off will still try to use it, and pick the partial-spend-avoiding version if it does not result in a
  fee increase over the non-avoiding version.
