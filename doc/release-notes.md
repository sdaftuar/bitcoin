...
Testnet support for Segregated Witness ("segwit"), as defined in [BIP
...
145](https://github.com/bitcoin/bips/blob/master/bip-0145.mediawiki) has been
included in this release.  Note that BIP 141 does not yet specify activation
parameters on mainnet; after BIP 141 is updated with proposed parameters,
a future release of Bitcoin Core is expected that implements those
parameters.
...
In preparation for future releases, the mining algorithm has been modified to
optimize transaction selection for a given block weight, rather than a given
...
For optimal performance, miners using this release should specify
...
behavior for both, may result in performance degradation, as the logic to
support `-blockmaxsize` performs additional computation to ensure that
constraint is met.  (Note that in this release, the equivalent parameter for
`-blockmaxweight` would be 4 times the desired `-blockmaxsize`.  See [BIP 141]
