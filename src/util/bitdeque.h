...
/** Class that mimicks std::deque<bool>, but with std::vector<bool>'s bit packing.
 *
...
class bitdeque
{
    ...
    public:
        ...
        /** Copy constructor, and conversion from non-const to const iterator. */
        template<bool ConstArg, typename = std::enable_if_t<Const || !ConstArg>>
        Iterator(const Iterator<ConstArg>& x) : m_it(x.m_it), m_bitpos(x.m_bitpos) {}
