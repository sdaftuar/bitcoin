        ...
        if (has_parent) {
            ...
            const auto parent_info = [&] {
                if (mempool_ancestors.size() > 0) {
                    ...
                    Assume(mempool_parent->GetMemPoolChildrenConst().size() == 0);
                    return ParentInfo{mempool_parent->GetTx().GetHash(),
                                      mempool_parent->GetTx().GetWitnessHash(),
                                      mempool_parent->GetTx().nVersion,
                                      /*has_mempool_descendant=*/mempool_parent->GetMemPoolChildrenConst().size() > 0};
                } else {
                                          ...
                                          int64_t vsize)
{
    ...
    if (mempool_ancestors.size() > 0) {
        ...
        if (parent_entry->GetMemPoolChildrenConst().size() + 2 > V3_DESCENDANT_LIMIT && !child_will_be_replaced) {
