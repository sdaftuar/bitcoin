...
struct ParentInfo {
    ...
    ParentInfo() = delete;
    ParentInfo(decltype(CTransaction::nVersion) version, const Txid& txid, const Wtxid& wtxid) :
        m_version{version}, m_txid{txid}, m_wtxid{wtxid}
    {}
};
        ...
        if (has_parent) {
            ...
            const auto parent_info = [&] {
                if (mempool_ancestors.size() > 0) {
                    ...
                    return ParentInfo{mempool_parent->GetTx().nVersion,
                                      mempool_parent->GetTx().GetHash(),
                    ...
                    return ParentInfo{package_parent->nVersion,
                                      package_parent->GetHash(),
            ...
            if (parent_info.m_version != 3) {
                return strprintf("v3 tx %s cannot spend from non-v3 tx %s",
                                 ptx->GetWitnessHash().ToString(), parent_info.m_wtxid.ToString());
            }
            ...
            for (const auto& package_tx : package) {
                ...
                for (auto& input : package_tx->vin) {
                    ...
                    // sibling is to-be-replaced (done in ApplyV3Rules) because that doesn't apply
                    // in a package.
