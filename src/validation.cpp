                                         ...
                                         PackageValidationState& package_state)
{
    ...
    if (const auto err_string{ImprovesFeerateDiagram(m_pool, direct_conflict_iters, m_all_conflicts, m_total_vsize,  m_total_modified_fees)}) {
        return package_state.Invalid(PackageValidationResult::PCKG_POLICY,
                                     "package RBF failed: " + *err_string, "");
    }
    m_conflicting_fees = 0;
    m_conflicting_size = 0;
    for (CTxMemPool::txiter it : m_all_conflicts) {
