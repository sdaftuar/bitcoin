...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    auto index = m_subpackage.m_changeset->AddTx(ptx, ws.m_base_fees, nAcceptTime, m_active_chainstate.m_chain.Height(), entry_sequence, fSpendsCoinbase, nSigOpsCost, lock_points.value());

    ws.m_vsize = m_subpackage.m_changeset->GetTxSize(index);
    ...
    if (auto ancestors{m_subpackage.m_changeset->CalculateMemPoolAncestors(index, maybe_rbf_limits)}) {
        ...
        if (auto ancestors_retry{m_subpackage.m_changeset->CalculateMemPoolAncestors(index, cpfp_carve_out_limits)}) {
...
bool MemPoolAccept::ReplacementChecks(Workspace& ws)
{
    ...
    for (auto it : m_subpackage.m_all_conflicts) {
        m_subpackage.m_changeset->RemoveTx(it);
    }
    return true;
}
...
void MemPoolAccept::FinalizeSubpackage(const ATMPArgs& args)
{
    ...
    for (CTxMemPool::txiter it : m_subpackage.m_all_conflicts)
    {
        LogPrint(BCLog::MEMPOOL, "replacing mempool tx %s (wtxid=%s, fees=%s, vsize=%s). ",
        ...
        if (m_subpackage.m_changeset->GetTxCount() == 1) {
            ...
            LogPrint(BCLog::MEMPOOL, "New tx %s (wtxid=%s, fees=%s, vsize=%s)\n",
                    tx.GetHash().ToString(),
                    tx.GetWitnessHash().ToString(),
                    feerate.fee, feerate.size);
        } else {
            LogPrint(BCLog::MEMPOOL, "New package with %lu txs, fees=%s, vsize=%s\n",
                                  ...
                                  std::map<uint256, MempoolAcceptResult>& results)
{
    ...
    for (Workspace& ws : workspaces) {
        if (!ConsensusScriptChecks(args, ws)) {
            ...
            m_subpackage.m_changeset->RemoveTx(m_pool.GetIter(ws.m_ptx->GetHash()).value());
        }
    }
    if (!all_submitted) {
