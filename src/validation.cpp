...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    ws.m_changeset->AddTx(ptx, ws.m_base_fees, nAcceptTime, m_active_chainstate.m_chain.Height(), entry_sequence, fSpendsCoinbase, nSigOpsCost, lock_points.value());

    ws.m_vsize = ws.m_changeset->GetTxSize(0);
    ...
    if (auto ancestors{ws.m_changeset->CalculateMemPoolAncestors(0, maybe_rbf_limits)}) {
        ...
        if (auto ancestors_retry{ws.m_changeset->CalculateMemPoolAncestors(0, cpfp_carve_out_limits)}) {
...
bool MemPoolAccept::Finalize(const ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : m_subpackage.m_all_conflicts)
    {
        LogPrint(BCLog::MEMPOOL, "replacing mempool tx %s (wtxid=%s, fees=%s, vsize=%s). New tx %s (wtxid=%s, fees=%s, vsize=%s)\n",
                ...
                ws.m_changeset->GetFee(0),
                ws.m_changeset->GetTxSize(0));
                                  ...
                                  std::map<uint256, MempoolAcceptResult>& results)
{
    ...
    for (Workspace& ws : workspaces) {
        ...
        {
            auto ancestors{ws.m_changeset->CalculateMemPoolAncestors(0, m_pool.m_opts.limits)};
            if(!ancestors) {
                results.emplace(ws.m_ptx->GetWitnessHash(), MempoolAcceptResult::Failure(ws.m_state));
