...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    if (showDebug)
    {
        ...
        strUsage += HelpMessageOpt("-bip91height=<n>", "Use specified BIP91 height (regtest-only)");
    }
...
bool AppInitParameterInteraction()
{
    ...
    if (IsArgSet("-bip91height")) {
        // Allow overriding BIP91 height for testing
        if (!chainparams.MineBlocksOnDemand()) {
            return InitError("BIP91 height may only be overridden on regtest.");
        }
        int64_t bip91_height = GetArg("-bip91height", 0);
        UpdateRegtestBIP91Height(bip91_height);
    }
