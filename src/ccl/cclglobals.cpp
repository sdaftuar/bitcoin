...
void CCLGlobals::SetupArgs(ArgsManager &gArgs)
{
    gArgs.AddArg("-dlogdir=<dir>", "Turn on data logging to specified output directory", ArgsManager::ALLOW_ANY, OptionsCategory::CCL);
    gArgs.AddArg("-simulation", "Sim mode! Don't call add networking threads to threadgroup", ArgsManager::ALLOW_ANY, OptionsCategory::CCL);
    gArgs.AddArg("-simdatadir=<dir>","For simulations: specify data directory (default: /chaincode/data/)", ArgsManager::ALLOW_ANY, OptionsCategory::CCL);
    gArgs.AddArg("-start=<YYYYMMDD>", "For simulations: start date", ArgsManager::ALLOW_INT, OptionsCategory::CCL);
    gArgs.AddArg("-end=<YYYYMMDD>", "For simulations: end date (defaults to start date)", ArgsManager::ALLOW_INT, OptionsCategory::CCL);
    gArgs.AddArg("-loadmempool=[1/0]", "Turn on/off loading initial mempool (default: 0)", ArgsManager::ALLOW_BOOL, OptionsCategory::CCL);
}
...
bool CCLGlobals::Init(ChainstateManager *chainman, CTxMemPool *mempool)
{
    m_chainman = chainman;
    m_mempool = mempool;
    ...
    if (gArgs.IsArgSet("-simulation")) {
        ...
        if (gArgs.GetBoolArg("-loadmempool", false)) {
            ...
            LoadMempool(*m_mempool, m_chainman->ActiveChainstate());
        }
    }
    return true;
}
...
bool CCLGlobals::Run()
{
    if (simulation.get() != NULL) {
        m_thread = std::thread(&util::TraceThread, "simulation", [&] { simulation->Run(); });
