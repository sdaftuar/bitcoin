...
void Simulation::RunSim(NodeContext& node)
{
    ...
    while (curdate <= enddate) {
        ...
        while (!txEOF || !blkEOF || !hdrEOF || !cbEOF || !btEOF) {
            if (node.chainman->m_interrupt) break;
            ...
            if (nextEvent == &txEvent) {
                ...
                BlockValidationState dummy;
                std::vector<CBlockHeader> headers = {cmpctblockEvent.obj->header};
                node.chainman->ProcessNewBlockHeaders(headers, true, dummy, NULL);
...
bool Simulation::ProcessOrphanTx(ChainstateManager &m_chainman)
{
    ...
    while (CTransactionRef porphanTx = m_orphanage->GetTxToReconsider(0)) {
        ...
        if (result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
            LogDebug(BCLog::MEMPOOL, "   accepted orphan tx %s\n", orphanHash.ToString());
            m_orphanage->AddChildrenToWorkSet(*porphanTx, rng);
            m_orphanage->EraseTx(porphanTx->GetWitnessHash());
            return true;
        } else if (state.GetResult() != TxValidationResult::TX_MISSING_INPUTS) {
            // Has inputs but not accepted to mempool
            // Probably non-standard or insufficient fee
            LogDebug(BCLog::MEMPOOL, "   removed orphan tx %s\n", orphanHash.ToString());
            m_orphanage->EraseTx(porphanTx->GetWitnessHash());
            return true;
        }
    }
...
void Simulation::ProcessTransaction(const CTransactionRef& ptx, ChainstateManager &m_chainman, CTxMemPool &m_mempool)
{
    ...
    if (result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
        LogDebug(BCLog::MEMPOOL, "AcceptToMemoryPool: accepted %s (poolsz %u txn, %u kB)\n",
                tx.GetHash().ToString(),
                m_mempool.size(), m_mempool.DynamicMemoryUsage() / 1000);
        std::set<uint256> orphan_work_set;
        m_orphanage->AddChildrenToWorkSet(tx, rng);
