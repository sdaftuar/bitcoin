...
public:
    ...
    typedef std::set<TxEntryRef, CompareById> Parents;
    typedef std::set<TxEntryRef, CompareById> Children;
...
public:
    ...
    ~Trimmer();
    void RemoveWorstChunk(std::vector<TxEntry::TxEntryRef>& txs_to_remove)

private:
    static bool ChunkCompare(const Cluster::HeapEntry& a, const Cluster::HeapEntry& b) const {
...
public:
    ...
    void AddTx(TxEntry *new_tx, int32_t vsize, CAmount modified_fee, const std::vector<TxEntryRef>& parents);

    // Lazily remove a transaction from the graph
    void RemoveTx(TxEntry *remove_tx) EXCLUSIVE_LOCKS_REQUIRED(cs);
    ...
    void AddParentTxs(std::vector<TxEntry::TxEntryRef> parent_txs, Limits limits, std::function<std::vector<TxEntry::TxEntryRef>()> func, std::vector<TxEntry *> &txs_removed);
