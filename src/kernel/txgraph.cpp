...
std::vector<TxEntry::TxEntryRef> InvokeSort(size_t tx_count, const std::vector<TxGraphCluster::Chunk>& chunks, bool reorg)
{
...
void TxGraphCluster::Sort(bool reassign_locations, bool reorg/*=false*/)
{
    // We can't assume the cluster is topologically valid if a reorg has just
    // occurred. Consider this scenario:
    //
    // Block: [A B C]. Tx C depends on A.
    // In mempool: Tx D, Tx E.
    // Tx D depends on B and A
    // Tx E depends on C and B
    // After readding block transactions, the mempool will have two new clusters:
    // [A C]
    // [B]
    // Then to stitch the mempool back together, we'll iterate in reverse over
    // the block transactions, and merge child clusters as we go:
    // After processing C, the mempool will look like:
    // [A C E] (1)
    // [B] (2)
    // [D] (3)
    // Then when we process B, we have two potential outcomes: we'll be trying to merge clusters
    // 1 and 3 into 2, but the order is not currently guaranteed, so we might end up in this outcome:
    // [B D A C E]
    // In which case the cluster will not be topologically valid (D is before A).
    // I'm not sure how to fix? Perhaps if we merged all the new clusters together first to make a super
    // cluster, then added the children, and then reclustered the result, that might work? In some ways that
    // would be nice, because maybe the algorithm is simpler to understand; on the other hand it might be slower
    // than what we do now, and the benefit to this seems limited?
    // assert(CheckTopo());
    std::vector<TxEntry::TxEntryRef> txs;
    if (m_tx_count <= 32) {
        txs = InvokeSort<BitSet<32>>(m_tx_count, m_chunks, reorg);
    } else if (m_tx_count <= 64) {
        txs = InvokeSort<BitSet<64>>(m_tx_count, m_chunks, reorg);
    } else if (m_tx_count <= 128) {
        txs = InvokeSort<BitSet<128>>(m_tx_count, m_chunks, reorg);
    } else if (m_tx_count <= 192) {
        txs = InvokeSort<BitSet<192>>(m_tx_count, m_chunks, reorg);
    } else if (m_tx_count <= 256) {
        txs = InvokeSort<BitSet<256>>(m_tx_count, m_chunks, reorg);
    } else if (m_tx_count <= 320) {
        txs = InvokeSort<BitSet<320>>(m_tx_count, m_chunks, reorg);
    } else if (m_tx_count <= 384) {
        txs = InvokeSort<BitSet<384>>(m_tx_count, m_chunks, reorg);
    } else if (m_tx_count <= 1280) {
        txs = InvokeSort<BitSet<1280>>(m_tx_count, m_chunks, reorg);
    } else {
...
void TxGraph::RecalculateTxGraphClusterAndMaybeSort(TxGraphCluster *cluster, bool sort, bool reorg/*=false*/)
{
...
// TODO: rewrite this to not use children, only parents. Right now, this is
// unsafe to invoke from TxGraphChangeSet, because during our RBF processing,
// the children are not consistent (only the parents).
        ...
        std::vector<TxEntry::TxEntryRef> &txs_removed)
{
    ...
    CheckMemory();
    // New algo:
    // Merge all new clusters together from the block.
    // Then merge all children in.
    // Then run reclustering, but don't sort.
    // Then limit everything that the new block transactions are in.
    TxGraphCluster *megacluster = nullptr;
    {
        std::vector<TxGraphCluster *> unique_clusters_from_block;
        WITH_FRESH_EPOCH(m_epoch);
        for (const auto& tx : parent_txs) {
            if (!visited(tx.get().m_cluster)) {
                unique_clusters_from_block.emplace_back(tx.get().m_cluster);
            }
        }
        if (unique_clusters_from_block.size() >= 1) {
            megacluster = unique_clusters_from_block[0];
            cachedInnerUsage -= megacluster->GetMemoryUsage();
            megacluster->Merge(unique_clusters_from_block.begin()+1, unique_clusters_from_block.end(), false);
            for (auto it = unique_clusters_from_block.begin()+1; it != unique_clusters_from_block.end(); ++it) {
                cachedInnerUsage -= (*it)->GetMemoryUsage();
                EraseCluster(*it);
            }
            cachedInnerUsage += megacluster->GetMemoryUsage();
            UpdateClusterIndex(megacluster);
        }
    }
    if (megacluster != nullptr) {
        for (const auto& tx : reverse_iterate(parent_txs)) {
            UpdateTxGraphClusterForDescendants(tx);
        }
        cachedInnerUsage -= megacluster->GetMemoryUsage();
        RecalculateTxGraphClusterAndMaybeSort(megacluster, false, true);
    }
    ...
    for (TxGraphCluster *cluster : unique_clusters_from_block) {
        ...
        if (cluster->m_tx_count > limits.cluster_count || cluster->m_tx_size > limits.cluster_size_vbytes) {
            ...
            RecalculateTxGraphClusterAndMaybeSort(cluster, true, true);
        } else {
            // Sort() can change the memory usage of the cluster
            cachedInnerUsage -= cluster->GetMemoryUsage();
            cluster->Sort(true, true);
            cachedInnerUsage += cluster->GetMemoryUsage();
            UpdateClusterIndex(cluster);
        }
    }
...
{
    AssertLockHeld(cs);
    TxEntry::TxEntryChildren children = tx.GetTxEntryChildren();
    CheckMemory();
    ...
    if (clusters_to_merge.size() > 1) {
        ...
        // first to try to make it more topologically sound (but this can't be guaranteed!)
        ...
        // Note that the topology may not be valid right now!
        UpdateClusterIndex(clusters_to_merge[0]);
    }
    cachedInnerUsage += clusters_to_merge[0]->GetMemoryUsage();
    return;
}
