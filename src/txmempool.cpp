...
void CTxMemPool::UpdateTransactionsFromBlock(const std::vector<uint256>& vHashesToUpdate)
{
    ...
    m_txgraph->Trim(); // Enforce cluster size limits.
    auto txs_to_remove = m_txgraph->Cleanup();
...
{
    m_txgraph = MakeTxGraph(m_opts.limits.cluster_count, m_opts.limits.cluster_size_vbytes);
}
...
CTxMemPool::ChangeSet::TxHandle CTxMemPool::ChangeSet::StageAddition(const CTransactionRef& tx, const CAmount fee, int64_t time, unsigned int entry_height, uint64_t entry_sequence, bool spends_coinbase, int64_t sigops_cost, LockPoints lp)
{
    ...
    TxGraph::Ref ref(m_pool->m_txgraph->AddTransaction(FeeFrac(fee+delta, GetVirtualTransactionSize(GetTransactionWeight(*tx), sigops_cost, ::nBytesPerSigOp))));
