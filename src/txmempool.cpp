                                    ...
                                    std::string &errString) const
{
    ...
    auto cluster_result{CheckClusterSizeLimit(total_size, package.size(), m_limits, staged_ancestors)};
    if (!cluster_result) {
...
bool CTxMemPool::BuildClusterForTransaction(CTxMemPoolEntry& entry, const setEntries& all_conflicts, const Limits& limits, Cluster& temp_cluster)
{
    ...
    temp_cluster.m_chunks[0].txs.push_back(entry);
    temp_cluster.m_tx_count++;
...
size_t CTxMemPool::DynamicMemoryUsage() const {
    ...
    return memusage::MallocUsage(sizeof(CTxMemPoolEntry) + 15 * sizeof(void*)) * mapTx.size() + memusage::DynamicUsage(mapNextTx) + memusage::DynamicUsage(mapDeltas) + memusage::DynamicUsage(vTxHashes) + memusage::DynamicUsage(m_cluster_map) + cachedInnerUsage;
}
...
std::vector<CTxMemPoolEntry::CTxMemPoolEntryRef> InvokeSort(bool reassign_locations, size_t tx_count, const std::vector<Cluster::Chunk>& chunks)
{
    ...
    if (tx_count >= 10) {
        ...
        LogPrint(BCLog::BENCH, "InvokeSort linearize cluster: %zu txs, %.4fms, %u iter, %.1fns/iter, %u comps, %.1fns/comp, encoding: %s\n",
                ...
                time_millis,
                result.iterations,
                time_millis * 1000000.0 / result.iterations,
                result.comparisons,
                time_millis * 1000000.0 / result.comparisons,
