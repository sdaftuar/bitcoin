...
public:
    ...
    void Trim() noexcept final;

    std::unique_ptr<BlockBuilder> GetBlockBuilder() noexcept final;
    std::vector<Ref*> GetWorstMainChunk() noexcept final;
...
uint64_t Cluster::AppendTrimData(std::vector<TrimTxData>& ret, std::vector<std::pair<GraphIndex, GraphIndex>>& deps) const noexcept
{
    ...
    for (unsigned i = 0; i < linchunking.NumChunksLeft(); ++i) {
        ...
        for (unsigned j = 0; j < chunk_tx_count; ++j) {
            ...
            entry.m_chunk_feerate = chunk_feerate;
...
void TxGraphImpl::GroupClusters() noexcept
{
    ...
    clusterset.m_group_data = std::vector<GroupData>{};
    clusterset.m_group_clusters.clear();
    clusterset.m_group_clusters.reserve(an_clusters.size());
    ...
    while (an_deps_it != an_deps.end()) {
        ...
        auto& new_entry = clusterset.m_group_data->emplace_back();
        new_entry.m_cluster_offset = clusterset.m_group_clusters.size();
...
void TxGraphImpl::ApplyDependencies() noexcept
{
    ...
    Assume(clusterset.m_group_data.has_value());
    for (const auto& group_data : *clusterset.m_group_data) {
        // Invoke Merge() to merge them into a single Cluster.
        auto cluster_span = std::span{clusterset.m_group_clusters}
...
void TxGraphImpl::AddDependency(const Ref& parent, const Ref& child) noexcept
{
    ...
    m_clustersets.back().m_group_data.reset();
    m_clustersets.back().m_group_clusters.clear();
...
void TxGraphImpl::StartStaging() noexcept
{
    ...
    stage.m_group_clusters = main.m_group_clusters;
...
void TxGraphImpl::CommitStaging() noexcept
{
    ...
    main.m_group_clusters = std::move(stage.m_group_clusters);
...
std::vector<TxGraph::Ref*> TxGraphImpl::GetWorstMainChunk() noexcept
{
    std::vector<Ref*> ret;
    // Make sure all clusters in main are up to date, and acceptable.
    SplitAll(0);
    ...
    if (!m_chunkindex.empty()) {
        const auto& chunk_data = *m_chunkindex.rbegin();
        ...
        if (chunk_data.m_chunk_count == LinearizationIndex(-1) || chunk_data.m_chunk_count == 1)  {
            ...
            ret.resize(1);
            Assume(chunk_end_entry.m_ref != nullptr);
            ret[0] = chunk_end_entry.m_ref;
        } else {
            ret.resize(chunk_data.m_chunk_count);
            auto start_pos = chunk_end_entry.m_main_lin_index + 1 - chunk_data.m_chunk_count;
            cluster->GetClusterRefs(*this, ret, start_pos);
...
void TxGraphImpl::Trim() noexcept
{
    int level = m_clustersets.size() - 1;
    Assume(m_chunkindex_observers == 0 || level > 0);
    ...
    for (const auto& group_data : *clusterset.m_group_data) {
        ...
        for (auto trim_it = trim_data.begin(); trim_it != trim_data.end(); ++trim_it) {
            ...
            // If this transaction has no unmet dependencies, add it to the heap (just append for
            // now, the heapification happens below).
