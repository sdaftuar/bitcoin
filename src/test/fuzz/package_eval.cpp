...
FUZZ_TARGET(tx_package_eval, .init = initialize_tx_pool)
{
    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 300)
    {
        ...
        // Whether a transaction was accepted to mempool (either through ProcessNewPackage or
        // AcceptToMemoryPool), and TrimToSize was called. This is not the same as passed, which can
        // indicate that a ATMP(test_accept=true) was successful.
        const bool accepted{!added.empty()};
        ...
        // Trimming is only done when submission happens, i.e. accepted=true. We may have called
        // PrioritiseTransaction on something in the mempool without calling TrimToSize.
        CheckMempoolV3Invariants(tx_pool, /*check_fees=*/accepted);
    }
