...
BOOST_FIXTURE_TEST_CASE(miniminer_negative, TestChain100Setup)
{
    ...
    auto result = node::FeeBumpCalculator::CalculateBumpFees(pool, {only_outpoint}, CFeeRate(0));
    BOOST_CHECK(result[only_outpoint] > 0);

    // With an sufficiently low target feerate, no fee bump should be required.
    result = node::FeeBumpCalculator::CalculateBumpFees(pool, {only_outpoint}, CFeeRate(-1000000));
    BOOST_CHECK(result[only_outpoint] == 0);
}
...
BOOST_FIXTURE_TEST_CASE(miniminer_1p1c, TestChain100Setup)
{
    ...
    for (const auto& feerate : various_normal_feerates) {
        auto bump_fees = node::FeeBumpCalculator::CalculateBumpFees(pool, nonexistent_outpoints, feerate);
    ...
    for (const auto& target_feerate : various_normal_feerates) {
        auto bump_fees = node::FeeBumpCalculator::CalculateBumpFees(pool, all_unspent_outpoints, target_feerate);
        BOOST_CHECK(sanity_check(all_transactions, bump_fees));
    ...
    for (const auto& target_feerate : various_normal_feerates) {
        auto bump_fees_all_spent = node::FeeBumpCalculator::CalculateBumpFees(pool, all_spent_outpoints, target_feerate);
        BOOST_CHECK_EQUAL(bump_fees_all_spent.size(), all_spent_outpoints.size());
        auto bump_fees_all_parents = node::FeeBumpCalculator::CalculateBumpFees(pool, all_parent_outputs, target_feerate);
 ...
 *  -> tx7_ASFR = (high+low+high) / (tx4_vsize + tx5_vsize + tx7_vsize) */
    ...
    // Extremely high feerate: everybody's bumpfee is from their full ancestor set.
    {
        ...
        auto bump_fees = node::FeeBumpCalculator::CalculateBumpFees(pool, all_unspent_outpoints, very_high_feerate);
        BOOST_CHECK_EQUAL(bump_fees.size(), all_unspent_outpoints.size());
        BOOST_CHECK(sanity_check(all_transactions, bump_fees));
        ...
        const auto tx3_bump_fee =
            node::FeeBumpCalculator::CalculateTotalBumpFees(pool,
                    {COutPoint{tx3->GetHash(), 0}, COutPoint{tx3->GetHash(), 1}},
                    very_high_feerate);
        BOOST_CHECK_EQUAL(tx3_bump_fee,
            very_high_feerate.GetFee(tx_vsizes[0] + tx_vsizes[1] + tx_vsizes[2] + tx_vsizes[3]) - (low_fee + med_fee + high_fee + high_fee));

        // Total fees: if spending both tx6 and tx7, don't double-count fees.
        const auto tx6_tx7_bumpfee =
            node::FeeBumpCalculator::CalculateTotalBumpFees(pool,
                    {COutPoint{tx6->GetHash(), 0}, COutPoint{tx7->GetHash(), 0}},
                    very_high_feerate);

        BOOST_CHECK_EQUAL(tx6_tx7_bumpfee,
    ...
    // Feerate just below tx4: tx6 and tx7 have different bump fees.
    {
        ...
        auto bump_fees = node::FeeBumpCalculator::CalculateBumpFees(pool, all_unspent_outpoints, just_below_tx4);
        BOOST_CHECK_EQUAL(bump_fees.size(), all_unspent_outpoints.size());
        BOOST_CHECK(sanity_check(all_transactions, bump_fees));
        const auto tx6_bumpfee = bump_fees.find(COutPoint{tx6->GetHash(), 0});
        BOOST_CHECK(tx6_bumpfee != bump_fees.end());
        ...
        // Total fees: if spending both tx6 and tx7, don't double-count fees.
        const auto tx6_tx7_bumpfee = node::FeeBumpCalculator::CalculateTotalBumpFees(pool,
                {COutPoint{tx6->GetHash(), 0}, COutPoint{tx7->GetHash(), 0}},
                just_below_tx4);
        BOOST_CHECK_EQUAL(tx6_tx7_bumpfee, just_below_tx4.GetFee(tx_vsizes[5] + tx_vsizes[6]) - (low_fee + med_fee));
    }
    // Feerate between tx6 and tx7's ancestor feerates: don't need to bump tx5 because tx7 already does.
    {
        ...
        auto bump_fees = node::FeeBumpCalculator::CalculateBumpFees(pool, all_unspent_outpoints, just_above_tx6);
        BOOST_CHECK_EQUAL(bump_fees.size(), all_unspent_outpoints.size());
        BOOST_CHECK(sanity_check(all_transactions, bump_fees));
        const auto tx6_bumpfee = bump_fees.find(COutPoint{tx6->GetHash(), 0});
        BOOST_CHECK(tx6_bumpfee != bump_fees.end());
