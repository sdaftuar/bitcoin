...
BOOST_FIXTURE_TEST_CASE(rbf_helper_functions, TestChain100Setup)
{
    ...
    opts.limits.cluster_size_vbytes = 1'000'000'250'000;
    opts.limits.cluster_count = 2300;
...
BOOST_FIXTURE_TEST_CASE(rbf_conflicts_calculator, TestChain100Setup)
{
    ...
    AddToMempool(pool, entry.Fee(normal_fee).FromTx(parent_tx));
    ...
    pool.removeForBlock({parent_entry->GetSharedTx()}, /* dummy */ 1);
