...
BOOST_AUTO_TEST_CASE(TxEntryTest)
{
    ...
    BOOST_CHECK_EQUAL(entry.parents.size(), 0);
    BOOST_CHECK_EQUAL(entry.children.size(), 0);
    ...
    entry2.parents.insert(entry);
    entry.children.insert(entry2);
    BOOST_CHECK_EQUAL(entry.children.size(), 1);
    BOOST_CHECK_EQUAL(entry2.parents.size(), 1);
}
...
BOOST_AUTO_TEST_CASE(TxGraphClusterAddRemoveTest)
{
    ...
    entry2.parents.erase(entry1);
    entry1.children.erase(entry2);
...
BOOST_AUTO_TEST_CASE(TxGraphClusterSortTest)
{
    ...
    for (int i=0; i < 30; ++i) {
        ...
        for (int j=0; j<i; ++j) {
            if (GetRand(4) == 0) {
                all_entries[i].parents.insert(all_entries[j]);
