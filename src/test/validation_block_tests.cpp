...
std::shared_ptr<CBlock> MinerTestingSetup::FinalizeBlock(std::shared_ptr<CBlock> pblock)
{
    ...
    BOOST_CHECK(Assert(m_node.chainman)->ProcessNewBlockHeaders({pblock->GetBlockHeader()}, ignored, true));
...
BOOST_AUTO_TEST_CASE(processnewblock_signals_ordering)
{
    ...
    BOOST_CHECK(Assert(m_node.chainman)->ProcessNewBlock(std::make_shared<CBlock>(Params().GenesisBlock()), true, &ignored, true));
    ...
    for (int i = 0; i < 10; i++) {
        threads.emplace_back([&]() {
            ...
            for (int i = 0; i < 1000; i++) {
                ...
                Assert(m_node.chainman)->ProcessNewBlock(block, true, &ignored, true);
            }

            // to make sure that eventually we process the full chain - do it here
            for (auto block : blocks) {
                if (block->vtx.size() == 1) {
                    bool processed = Assert(m_node.chainman)->ProcessNewBlock(block, true, &ignored, true);
                    assert(processed);
                }
            }
        });
...
BOOST_AUTO_TEST_CASE(mempool_locks_reorg)
{
    ...
    auto ProcessBlock = [&](std::shared_ptr<const CBlock> block) -> bool {
        return Assert(m_node.chainman)->ProcessNewBlock(block, /*force_processing=*/true, /*new_block=*/&ignored, true);
    };
